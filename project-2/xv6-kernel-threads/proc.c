...
struct {
  struct spinlock lock;
  struct mutex mutex[NPROC]; // MAX_MUTEXES ?
} mtable;
...
void kill_others()
{
	...
	for(t = proc->threads; t < &proc->threads[NTHREAD]; t++) {
		if (t->tid != thread->tid && t->state != TRUNNING && t->state != TUNUSED) {
			t->state = TZOMBIE;
		}
	}
...
// 8 new kthread functions by Nicolas and Sean
...
int kthread_create(void*(*start_func)(), void* stack, int stack_size) {
	...
	if (t == 0) {
		...
		t->state = TRUNNABLE;
		*t->tf = *thread->tf;
		uint stack_address = (uint)stack;
		t->tf->esp = stack_address + (uint)stack_size;
		t->tf->ebp = t->tf->esp;
		uint func_address = (uint)start_func;
		t->tf->eip = func_address;
		return t->tid;
	}
}

int kthread_id() {
	if ((!proc) || (!thread) || proc->pid < 0 || thread->tid < 0) return -1;
	return thread->tid;
}
...
void kthread_exit() {
	...
	for(t = proc->threads; found != 1 && t < &proc->threads[NTHREAD]; t++) {
		if (t->tid != thread->tid && (t->state != TUNUSED && t->state != TZOMBIE && t->state != TINVALID)) { 
			found = 1;
			break;
		}
	}
...
int kthread_join(int thread_id) {
	...
	if (thread->tid == thread_id || thread_id < 0) return -1;
	...
	// let t finish execution
	if (t->tid == thread_id && (t->state != TINVALID && t->state != TZOMBIE && t->state != TUNUSED)) {
		sleep(t, &ptable.lock);
	}
	if (t->state == TZOMBIE) {
		clearThread(t);
...
int kthread_mutex_lock(int mutex_id) {
	...
	if (i == MAX_MUTEXES || m->state == MUNUSED) {
		release(&mtable.lock);
		return -1;
	}
